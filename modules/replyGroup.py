...
@channel.use(ListenerSchema(listening_events=[GroupMessage], decorators=[MatchContent("1")]))
async def ero2(app: Ariadne, group: Group, member: Member):
    ...
    inc = InterruptControl(app.broadcast)
    try:
        ret_msg = await inc.wait(setu_tag_waiter,timeout=10)
    except asyncio.exceptions.TimeoutError:
        await app.sendGroupMessage(group, MessageChain.create("已超时取消"))

    await app.sendGroupMessage(group, MessageChain.create(f"{ret_msg}"))